...
def update_ball_position_multi(game_state):
	...
	if 'collision_counter' not in game_state['multi']:
		game_state['multi']['collision_counter'] = 0
	if 'last_collision_time' not in game_state['multi']:
		game_state['multi']['last_collision_time'] = time.time()

	current_time = time.time()

	# Reset the collision counter if more than 0.5 second has passed since the last collision
	if current_time - game_state['multi']['last_collision_time'] > 0.5:
		game_state['multi']['collision_counter'] = 0
		game_state['multi']['last_collision_time'] = current_time
	...
	for i in range(len(players)):
		...
		if collision_with_player:
			...
			if game_state['multi']['collision_counter'] >= 4:
				#reverse the tmp ball direction
		
				direction['x'] = -game_state['multi']['temp_dir']['x']
				direction['y'] = -game_state['multi']['temp_dir']['y']
				ball['x'] += direction['x'] * speed
				ball['y'] += direction['y'] * speed

				game_state['multi']['ball_direction'] = direction
				game_state['multi']['ball_position'] = ball
				game_state['multi']['collision_counter'] = 0 #reset the collision counter
