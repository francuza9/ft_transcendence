...
export function initCornerLights(targ)
{
	const tempGroup = new THREE.Group();

	// creating cylinders on the corners
	const geometryCorners = new THREE.CylinderGeometry(0.2,0.2,3.4,32);
	const materialCorners = new THREE.MeshStandardMaterial( {color: 0x222222});
	...
	const light1 = new THREE.SpotLight(0xffffff, 1000, 0);
	light1.position.set(-5.7, 5.7, -4.2);
	light1.angle = 0.4;
	light1.castShadow = true;
	light1.target = targ;
	const light2 = new THREE.SpotLight(0xffffff, 1000, 0);
	light2.position.set(-5.7, 5.7, 4.2);
	light2.angle = 0.4;
	light2.castShadow = true;
	light2.target = targ;
	const light3 = new THREE.SpotLight(0xffffff, 1000, 0);
	light3.position.set(5.7, 5.7, 4.2);
	light3.angle = 0.4;
	light3.castShadow = true;
	light3.target = targ;
	const light4 = new THREE.SpotLight(0xffffff, 1000, 0);
	...
	// create 'lightsource' (just white circles on projectors)
	const geometryL = new THREE.SphereGeometry(0.15, 32, 32);
	const materialL = new THREE.MeshLambertMaterial( {color:0xffffff, emissive:0xffffff} );

	const L1 = new THREE.Mesh(geometryL, materialL);
	L1.position.x = -5.55;
	L1.position.y = 3.9;
	L1.position.z = -4.2;
	const L2 = new THREE.Mesh(geometryL, materialL);
	L2.position.x = 5.55;
	L2.position.y = 3.9;
	L2.position.z = 4.2;
	const L3 = new THREE.Mesh(geometryL, materialL);
	L3.position.x = 5.55;
	L3.position.y = 3.9;
	L3.position.z = -4.2;
	const L4 = new THREE.Mesh(geometryL, materialL);
	L4.position.x = -5.55;
	L4.position.y = 3.9;
	L4.position.z = 4.2;
